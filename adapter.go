...
import (
	...
	"cfa/native/vkturn/pkg/clientcore"
)

func ParseConfig(args []string) (clientcore.Config, error) {
	...
	flags.IntVar(&cfg.StreamsPerCred, "streams-per-cred", clientcore.DefaultStreamsPerCred(), "number of TURN streams sharing one VK credential cache")
	...
	if err := flags.Parse(args); err != nil {
		return clientcore.Config{}, err
	}
	if *genWrapKey {
		key, err := clientcore.GenWrapKeyHex()
		if err != nil {
			return clientcore.Config{}, err
		}
		cfg.WrapKeyHex = key
